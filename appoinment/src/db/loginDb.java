...
public class loginDb {
        public static boolean loginUser(String username, String password) {
            try {
                            ...
                            PreparedStatement insertUser = connection.prepareStatement("SELECT * FROM " + commonconstant.DB_TABLE_NAME + " WHERE " + " User_email = ? and password = ?");
                            insertUser.setString(1, username);
                            insertUser.setString(2, password);

                            ResultSet rowsInserted = insertUser.getResultSet();
                            if(!Objects.isNull(rowsInserted)){
                                return true;
                            }
