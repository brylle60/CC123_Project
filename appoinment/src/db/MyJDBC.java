...
public class MyJDBC {
    ...
    public static boolean register(String last_name, String first_name, String middle_name, int age, int number, String email, String password, String address, LocalDate birthdate, Boolean loggin){
        try{
    ...
    public static List<User> getLoggedInUsers() {
        ...
        try {
            Connection connection = DriverManager.getConnection(commonconstant.DB_URL, commonconstant.DB_USERNAME, commonconstant.DB_PASSWORD);
            PreparedStatement statement = connection.prepareStatement("SELECT last_name, first_name, middle_name, age, mobile_number, User_email, user_password, logged_in_users FROM " + commonconstant.DB_TABLE_NAME + " WHERE logged_in_users = 1");
